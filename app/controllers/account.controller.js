...
exports.forgotPassword = (req, res) => {
  ...
  db.User.findOne({
    ...
    const token = jwt.sign(dataToSign, config.resetPassword, { expiresIn: '2m' });
    const data = {
      to: email,
  ...
  if (resetLink) {
    jwt.verify(resetLink, config.resetPassword, (err, decodeData) => {
      ...
      db.User.findOne({ resetLink }, (e, user) => {
        ...
        user.updateOne({ password: bcrypt.hashSync(newPassword, 8) }, (err, success) => {
          if (err) {
            return res.status(500).json({ message: "Can not update new password" });
          }
          else {
