...
exports.findAll = async (req, res) => {
  try {
    ...
    const length = await Comment.find().countDocuments()
...
exports.findByID= async (req, res) => {
  try {
    const {id} = req.params;
    const category = await Comment.find({_id:id}).populate({path:'authorID'})
    if (category) {
...
exports.create = async (req, res) => {
  ...
  try {
    // console.log("BODY",req.body)
      ...
      //Comment.setPasswordHash(password)
       console.log(comment);
...
exports.update = async (req, res) => {
  ...
  try {
      ...
      const category = await Comment.findOne({ _id })

      if(category) {

          const result = await Comment.findOneAndUpdate({ _id }, 
                                                        { title: title || comment.title,
                                                          content: content || comment.content,
                                                          datePosted: datePosted || comment.datePosted,
                                                          like: status })
          if (result) {
              const data = await Comment.findOne({ _id: result._id })
              if (data) {
